...
public class QueensEnv extends Environment {
    ...
    class QueensModel extends GridWorldModel {
		...
		void putQueen() {
            Location r1 = getAgPos(0);
            add(QUEEN, r1);
			Literal queen = Literal.parseLiteral("queen(" + r1.x + "," + r1.y + ")");
			addPercept(queen);
			queensPlaced++;
			Literal turn = Literal.parseLiteral("player(" + queensPlaced%2 + ")");
			addPercept(turn);
			//cellAttacked(r1.x,r1.y);
        }
    ...
    class QueensView extends GridWorldView {
        ...
        public void drawQueen(Graphics g, int x, int y) {
			...
			//if ((x+y)%2==0){
			if (queensPlaced%2==0){
				g.drawImage(bqImg, x*48, y*45, null);
			} else {
				g.drawImage(wqImg, x*48, y*45, null);
			};
