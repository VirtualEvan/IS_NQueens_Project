...
pasaTurno :-
	movs(NM) &
	maxQueens(NQ) &
	.print("Configurador pasa turno.") &
	NM <= NQ/2.

puedeMover :-
	.print("Configurador puede mover.") &
	maxBlocks(NB) &
	maxHoles(NH) &
	maxQueens(NQ) &
	NB+NH>NQ/2.
...
+!selectBlockMov : free(_,_) & pasaTurno <-
	.print("El configurador pasa turno.").

+!selectBlockMov : free(_,_) & not pasaTurno & not puedeMover <-//maxBlocks(NB) & maxHoles(NH) & maxQueens(NQ) & NB+NH<=NQ/2 <-
	.print("No tengo más movimientos permitidos.").

+!selectBlockMov : free(_,_) & not pasaTurno & puedeMover <-//maxBlocks(NB) & maxHoles(NH) & maxQueens(NQ) & NB+NH>NQ/2 <-
	...
	if (accept(Player1) & accept(Player2) & not Player1==Player2) {
		block(X,Y);
		.abolish(accept(_));
	} else {
		if (not accepted(_)) {
